(ns postprodops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (PostProdAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the PostProdGovernor (:govern)
  ...
  human operator/post-production coordinator. The approver resumes with
...
(defn build
  ...
  `postprodops.store/Store`).
  opts:
    :advisor      -- a `postprodops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PostProdAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; PostProdGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :project-id (:project-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :project-id (:project-id request) :by (:by approval)}]}
            {:disposition :hold
