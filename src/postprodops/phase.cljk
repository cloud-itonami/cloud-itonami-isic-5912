(ns postprodops.phase
  "Phase 0->3 staged rollout for the ISIC-5912 motion-picture/video/
  television post-production operations-coordination actor.
    ...
    Phase 2  assisted-coordination-- adds production-operation scheduling
                                     and delivery coordination, still
                                     approval.
  ...
  needs a human to actually look at it. `postprodops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently --
  two layers, not one, agree on this."
  (:require [postprodops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                                       :auto #{}}
   ...
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-production-record :schedule-production-operation :coordinate-delivery}}})
...
(defn verdict->disposition
  "Map a PostProdGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
